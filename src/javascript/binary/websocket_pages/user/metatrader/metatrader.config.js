...
const MetaTraderConfig = (() => {
    ...
    const types_info = {
        ...
        vanuatu_cent: {
            ...
            mt5_account_type: 'cent',
            title           : localize('Real Cent'),
            max_leverage    : 1000,
        },
        vanuatu_standard: {
            account_type    : 'financial',
        ...
        vanuatu_stp: {
            account_type    : 'financial',
            mt5_account_type: 'stp',
            title           : localize('Real STP'),
            max_leverage    : 100,
        },
        costarica: {
            account_type    : 'gaming',
            mt5_account_type: '',
            title           : localize('Real Volatility'),
            max_leverage    : 100,
        },
    };

    const needsRealMessage = () =>
        $(`#msg_${Client.get('has_real') ? 'switch' : 'upgrade'}`).html();
    ...
    const actions_info = {
        new_account: {
            ...
            success_msg: (response) => {
                ...
                switch (acc_type) {
                    case 'financial':
                        acc_type = `vanuatu_${response.mt5_new_account.mt5_account_type}`;
                        break;
                    case 'gaming':
                        acc_type = 'costarica';
                        break;
            ...
            prerequisites: acc_type =>
                new Promise((resolve) => {
                    if (types_info[acc_type].is_demo) {
                        ...
                        types_info[acc_type].account_type === 'financial'
                    ) {
                        BinarySocket.send({
                            get_financial_assessment: 1,
                        }).then((response_financial) => {
                            resolve(
                                isEmptyObject(
                                    response_financial.get_financial_assessment,
                                )
                                    ? $('#msg_assessment')
                                          .find('a')
                                          .attr(
                                              'onclick',
                                              `localStorage.setItem('financial_assessment_redirect', '${urlFor('user/metatrader')}')`,
                                          )
                                          .end()
                                          .html()
                                    : '',
            ...
            formValues: ($form, acc_type, action) => {
                ...
                $form
                    .find(fields[action].lbl_account_type.id)
                    .text(types_info[acc_type].title);
                // Email
                $form
                    .find(fields[action].lbl_email.id)
                    .text(fields[action].additional_fields(acc_type).email);
                // Max leverage
                $form
                    .find(`${fields[action].ddl_leverage.id} option`)
                    .each(function() {
                        if (
                            +$(this).val() > types_info[acc_type].max_leverage
                        ) {
                            $(this).remove();
                        }
                    });
        ...
        password_change: {
            ...
            success_msg: response =>
                localize(
                    ...
                    [response.echo_req.login],
                ),
            prerequisites: () => new Promise(resolve => resolve('')),
            formValues   : ($form, acc_type, action) => {
                // Login ID
                $form
                    .find(fields[action].lbl_login.id)
                    .text(fields[action].additional_fields(acc_type).login);
            },
        },
        deposit: {
                            ...
                            if (
                                ...
                                resolve(
                                    localize(
                                        ...
                                        [
                                            urlFor(
                                                'user/security/cashier_passwordws',
                                            ),
                                        ],
                                    ),
                                );
            ...
            formValues: ($form, acc_type, action) => {
                // From, To
                $form
                    .find(fields[action].lbl_from.id)
                    .text(
                        fields[action].additional_fields(acc_type).from_binary,
                    );
                $form
                    .find(fields[action].lbl_to.id)
                    .text(fields[action].additional_fields(acc_type).to_mt5);
            },
        },
        withdrawal: {
            ...
            pre_submit: ($form, acc_type, displayFormMessage) =>
                BinarySocket.send({
                    ...
                    password          : $form
                        .find(fields.withdrawal.txt_main_pass.id)
                        .val(),
                }).then((response) => {
            ...
            formValues: ($form, acc_type, action) => {
                ...
                $form
                    .find(fields[action].lbl_from.id)
                    .text(fields[action].additional_fields(acc_type).from_mt5);
                $form
                    .find(fields[action].lbl_to.id)
                    .text(fields[action].additional_fields(acc_type).to_binary);
            },
        },
    };
    ...
    const fields = {
        new_account: {
            ...
            txt_main_pass   : {
                id           : '#txt_main_pass',
                request_field: 'mainPassword',
            },
            txt_re_main_pass : { id: '#txt_re_main_pass' },
            txt_investor_pass: {
                id           : '#txt_investor_pass',
                request_field: 'investPassword',
            },
            chk_tnc          : { id: '#chk_tnc' },
            additional_fields: acc_type =>
                $.extend(
                    ...
                    types_info[acc_type].mt5_account_type
                        ? {
                            mt5_account_type: types_info[acc_type]
                                  .mt5_account_type,
                        }
                        : {},
                ),
        ...
        password_change: {
            ...
            txt_old_password: {
                id           : '#txt_old_password',
                request_field: 'old_password',
            },
            txt_new_password: {
                id           : '#txt_new_password',
                request_field: 'new_password',
            },
    ...
    const validations = {
        new_account: [
            {
                selector   : fields.new_account.txt_name.id,
                validations: [
                    'req',
                    'letter_symbol',
                    ['length', { min: 2, max: 30 }],
            ...
            {
                selector   : fields.new_account.txt_main_pass.id,
                validations: ['req', 'password'],
            },
            {
                selector   : fields.new_account.txt_re_main_pass.id,
                validations: [
                    'req',
                    ['compare', { to: fields.new_account.txt_main_pass.id }],
                ],
            },
            {
                selector   : fields.new_account.txt_investor_pass.id,
                validations: [
                    'req',
                    'password',
                    [
                        'not_equal',
                        {
                            to   : fields.new_account.txt_main_pass.id,
                            name1: 'Main password',
                            name2: 'Investor password',
                        },
                    ],
                ],
            },
            {
                selector   : fields.new_account.ddl_leverage.id,
                validations: ['req'],
            },
            {
                selector   : fields.new_account.chk_tnc.id,
                validations: [
                    [
                        'req',
        ...
        password_change: [
            {
                selector   : fields.password_change.txt_old_password.id,
                validations: ['req'],
            },
            {
                selector   : fields.password_change.txt_new_password.id,
                validations: [
                    'req',
                    'password',
                    [
                        'not_equal',
                        {
                            to   : fields.password_change.txt_old_password.id,
                            name1: 'Current password',
                            name2: 'New password',
                        },
                    ],
                ],
                re_check_field: fields.password_change.txt_re_new_password.id,
            },
            {
                selector   : fields.password_change.txt_re_new_password.id,
                validations: [
                    'req',
                    [
                        'compare',
                        { to: fields.password_change.txt_new_password.id },
                    ],
                ],
            },
        ],
        deposit: [
            {
                ...
                validations: [
                    'req',
                    ['number', { type: 'float', min: 1, max: 20000 }],
                ],
            },
        ],
        withdrawal: [
            {
                selector   : fields.withdrawal.txt_main_pass.id,
                validations: ['req'],
            },
            {
