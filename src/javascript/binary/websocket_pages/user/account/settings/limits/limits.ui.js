...
const LimitsUI = (() => {
    ...
    const appendRowTable = (name, turnover_limit, padding, font_weight) => {
        client_limits.append(
            $('<tr/>', { class: 'flex-tr' })
                .append(
                    $('<td/>', {
                        class: 'flex-tr-child',
                        style: `padding-left: ${padding}; font-weight: ${font_weight};`,
                        text : localize(name),
                    }),
                )
    ...
    const fillLimitsTable = (limits) => {
        ...
        const payout_per = document.getElementById(
            'payout-per-symbol-and-contract-type',
        );

        elementTextContent(
            open_position,
            addComma(limits.open_positions).split('.')[0],
        );
        elementTextContent(
            account_balance,
            addComma(limits.account_balance).split('.')[0],
        );
        elementTextContent(payout, addComma(limits.payout).split('.')[0]);
        elementTextContent(
            payout_per,
            addComma(limits.payout_per_symbol_and_contract_type).split('.')[0],
        );
        ...
        Object.keys(market_specific).forEach((key) => {
            ...
            if (object.length && object.length > 0) {
                appendRowTable(
                    localize(key.charAt(0).toUpperCase() + key.slice(1)),
                    '',
                    'auto',
                    'bold',
                );
                Object.keys(object).forEach((c) => {
                    if (
                        ...
                        /Major Pairs/.test(object[c].name)
                    ) {
                        appendRowTable(
                            object[c].name,
                            object[c].turnover_limit !== 'null'
                                ? addComma(object[c].turnover_limit).split('.')[
                                      0
                                  ]
                                : 0,
                            '25px',
                ...
                appendRowTable(
                    object.name,
                    object.turnover_limit !== 'null'
                        ? addComma(object.turnover_limit).split('.')[0]
                        : 0,
                    'auto',
                    'bold',
                );
