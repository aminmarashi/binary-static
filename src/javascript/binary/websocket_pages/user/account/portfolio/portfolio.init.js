...
const PortfolioInit = (() => {
    ...
    const init = () => {
        ...
        BinarySocket.send(
            ...
            { callback: transactionResponseHandler },
        );
    ...
    const createPortfolioRow = (data, is_first) => {
        const long_code = typeof module !== 'undefined'
            ? data.longcode
            : jpClient()
                  ? toJapanTimeIfNeeded(undefined, undefined, data.longcode)
                  : data.longcode;
        ...
        $div
            .append(
                $('<tr/>', {
                    class: `tr-first ${new_class} ${data.contract_id}`,
                    id   : data.contract_id,
                })
                    .append(
                        $('<td/>', { class: 'ref' }).append(
                            $(
                                `<span ${GetAppDetails.showTooltip(data.app_id, oauth_apps[data.app_id])} data-balloon-position="right">${data.transaction_id}</span>`,
                            ),
                        ),
                    )
                    .append(
                        $('<td/>', { class: 'payout' }).append(
                            $('<strong/>', {
                                text: formatMoney(data.currency, data.payout),
                            }),
                        ),
                    )
                    .append($('<td/>', { class: 'details', text: long_code }))
                    .append(
                        $('<td/>', { class: 'purchase' }).append(
                            $('<strong/>', {
                                text: formatMoney(
                                    data.currency,
                                    data.buy_price,
                    ...
                    .append(
                        $('<td/>', { class: 'indicative' }).append(
                            $('<strong/>', {
                                class: 'indicative_price',
                                text : '--.--',
                            }),
                        ),
                    )
                    .append(
                        $('<td/>', { class: 'button' }).append(
                            $('<button/>', {
                                class      : 'button open_contract_details nowrap',
            ...
            .append(
                $('<tr/>', {
                    class: `tr-desc ${new_class} ${data.contract_id}`,
                }).append($('<td/>', { colspan: '6', text: long_code })),
            );
    ...
    const updateBalance = () => {
        ...
        $portfolio_balance.text(
            Portfolio.getBalance(Client.get('balance'), Client.get('currency')),
        );
    ...
    const updateIndicative = (data) => {
        ...
        const proposal = Portfolio.getProposalOpenContract(
        ...
        if (+proposal.is_sold === 1) {
            ...
            if (+proposal.is_valid_to_sell !== 1) {
                no_resale_html = $('<span/>', {
                    text: localize('Resale not offered'),
                });
                $td.addClass('no_resale');
            } else {
                status_class = values[proposal.contract_id].indicative <
                    old_indicative
                    ? ' price_moved_down'
                    : values[proposal.contract_id].indicative > old_indicative
                          ? ' price_moved_up'
                          : '';
            ...
            $td.html(
                $('<strong/>', {
                    class: `indicative_price ${status_class}`,
                    text : formatMoney(
                        proposal.currency,
                        values[proposal.contract_id].indicative,
                    ),
    ...
    const updateFooter = () => {
        $('#cost-of-open-positions').text(
            formatMoney(currency, Portfolio.getSumPurchase(values)),
        );
        $('#value-of-open-positions').text(
            formatMoney(currency, Portfolio.getIndicativeSum(values)),
        );
    };
