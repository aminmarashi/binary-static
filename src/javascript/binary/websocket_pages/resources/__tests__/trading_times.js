...
describe('Trading Times', () => {
    ...
    it('Should have all functions that are being tested', () => {
        expect(trading_times).to.have.all.of.keys([
            'getSubmarketInfo',
            'getSymbolInfo',
        ]);
    });
    ...
    it('Should trading_times and getSubmarketInfo() have all expected data', () => {
        expect(trading_times_res).to.be
            .an('object')
        ...
        trading_times_res.markets.forEach((market) => {
            ...
            market.submarkets.forEach((submarket) => {
                expect(submarket).to.be
                    .an('object')
                    .that.have.property('name')
                    .that.is.a('string');
                expect(submarket).to.have
                    .property('symbols')
                    .that.is.an('array');

                const submarket_info = trading_times.getSubmarketInfo(
                    active_symbols_res,
                    submarket.name,
                );
