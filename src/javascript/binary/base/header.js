...
const Header = (() => {
    ...
    const upgradeMessageVisibility = () => {
        BinarySocket.wait(
            ...
            if (Client.get('is_virtual')) {
                ...
                const jp_account_status = (State.get([
                    'response',
                    'get_settings',
                    'get_settings',
                    'jp_account_status',
                            ...
                            if ($('.activation-message').length === 0) {
                                $('#virtual-text').append(
                                    $('<div/>', {
                                        ...
                                        text : ` ${localize('Your Application is Being Processed.')}`,
                                    }),
                                );
                            }
                            ...
                            if ($('.activated-message').length === 0) {
                                $('#virtual-text').append(
                                    $('<div/>', {
                                        ...
                                        text : ` ${localize('{JAPAN ONLY}Your Application has Been Processed. Please Re-Login to Access Your Real-Money Account.')}`,
                                    }),
                                );
                            }
                        }
    ...
    const hideNotification = () => {
        const $msg_notification = $('#msg_notification');
        if ($msg_notification.is(':visible')) {
    ...
    const displayAccountStatus = () => {
        BinarySocket.wait('authorize').then(() => {
            ...
            const buildMessage = (string, path, hash = '') =>
                localize(string, [`<a href="${urlFor(path)}${hash}">`, '</a>']);

            const messages = {
                authenticate: () =>
                    buildMessage(
                        '[_1]Authenticate your account[_2] now to take full advantage of all withdrawal options available.',
                        'user/authenticate',
                    ),
                financial_limit: () =>
                    buildMessage(
                        'Please set your 30-day turnover limit in our [_1]self-exclusion facilities[_2] to remove deposit limits.',
                        'user/security/self_exclusionws',
                        '#max_30day_turnover',
                    ),
                residence: () =>
                    buildMessage(
            ...
            const check_statuses_real = [
                ...
                {
                    validation: validations.financial_limit,
                    message   : messages.financial_limit,
                },
                { validation: validations.risk, message: messages.risk },
                { validation: validations.tax, message: messages.tax },
                {
                    validation: validations.authenticate,
                    message   : messages.authenticate,
                },
                {
                    validation: validations.unwelcome,
                    message   : messages.unwelcome,
                },
            ];
