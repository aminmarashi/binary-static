...
const compareBigUnsignedInt = require('../common_functions/string_util')
...
const Validation = (() => {
                ...
                fields.forEach((field) => {
                    ...
                    if (field.msg_element) {
                        ...
                        if ($parent.find(`div.${error_class}`).length === 0) {
                            $parent.append(
                                $('<div/>', {
                                    class: `${error_class} ${hidden_class}`,
                                }),
                            );
                        }
                    ...
                    if (event) {
                        field.$.unbind(event).on(event, () => {
                            ...
                            if (field.re_check_field) {
                                checkField(
                                    forms[form_selector].fields.find(
                                        fld =>
                                            fld.selector ===
                                            field.re_check_field,
                                    ),
                                );
                            }
                        });
    ...
    const validRequired = (value, options, field) => {
        ...
        // else
        validators_map.req.message = field.type === 'checkbox'
            ? 'Please select the checkbox.'
            : 'This field is required.';
        return false;
    };
    const validEmail = value =>
        /^[a-zA-Z0-9_.+-]+@[a-zA-Z0-9.-]+\.[a-zA-Z]{2,63}$/.test(value);
    const validPassword = value =>
        /(?=.*[a-z])(?=.*[A-Z])(?=.*\d)[a-zA-Z\d]+/.test(value);
    const validLetterSymbol = value =>
        !/[`~!@#$%^&*)(_=+\[}{\]\\\/";:\?><,|\d]+/.test(value);
    const validGeneral = value =>
        !/[`~!@#$%^&*)(_=+\[}{\]\\\/";:\?><|]+/.test(value);
    ...
    const validNumber = (value, options) => {
        ...
        if (
            ...
            options.decimals &&
            !new RegExp(
                `^\\d+(\\.\\d{${options.decimals.replace(/ /g, '')}})?$`,
            ).test(value)
        ) {
            is_ok = false;
            message = localize('Only [_1] decimal points are allowed.', [
                options.decimals,
            ]);
        } else if (
            'min' in options &&
            ...
            is_ok = false;
            message = localize('Should be between [_1] and [_2]', [
                options.min,
                options.max,
            ]);
    ...
    const isMoreThanMax = (value, options) =>
        (options.type === 'float'
            ? +value > +options.max
            : compareBigUnsignedInt(value, options.max) === 1);
    ...
    const validators_map = {
        ...
        password: {
            func   : validPassword,
            message: 'Password should have lower and uppercase letters with numbers.',
        },
        general: {
            func   : validGeneral,
        ...
        email_token: {
            func   : validEmailToken,
            message: 'Please submit a valid verification token.',
        },
        compare: {
            func   : validCompare,
        ...
        not_equal: {
            func   : validNotEqual,
            message: '[_1] and [_2] cannot be the same.',
        },
        min: {
            func   : validMin,
            message: 'Minimum of [_1] characters required.',
        },
        length: {
            func   : validLength,
            message: 'You should enter [_1] characters.',
        },
    ...
    const checkField = (field) => {
        ...
        field.validations.some((valid) => {
            ...
            if (
                type === 'password' &&
                !validLength(getFieldValue(field), pass_length)
            ) {
                field.is_ok = false;
                type = 'length';
                options = pass_length;
            } else {
                const validator = type === 'custom'
                    ? options.func
                    : validators_map[type].func;
            ...
            if (!field.is_ok) {
                ...
                if (type === 'length') {
                    ...
                    message = localize(message, [
                        localize(options.name1),
                        localize(options.name2),
                    ]);
                }
                all_is_ok = false;
                return true;
